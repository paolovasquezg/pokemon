  ...
  <div className="flex flex-col bg-gray-500 rounded-lg border-4 border-white p-4 sm:p-8 md:p-10 w-full max-w-3xl mx-auto max-h-[85vh] gap-6 sm:mt-0">
    <div className='absolute top-0 left-0'>
        ...
        {info ? (
          ...
          <div className="max-h-[68vh] max-[800px]:overflow-y-auto">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 mt-4 sm:mt-10 ml-0 place-items-center">
              {Object.entries(type_data).map(([name, { src, color }]) => (
                <div key={name} className={`rounded-full p-4 flex flex-col items-center justify-center w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 min-w-0 ${loading ? "animate-spin" : ""}`} style={{ backgroundColor: color }}>
        ...
        {error && <p className='text-center font-mono text-white mt-4'>Type not found</p>}

        <div className='flex flex-row justify-center gap-x-2 mt-auto pt-4'>
          <input
            type="text"
            value={type}
            ...
            title="Pokémon type"
            className='font-mono bg-white rounded-sm h-10 p-2'/>
          <button
